...
import HRUserTrainingForm from "./HRUserTrainingForm"; // Form component import for HR

// Main Functional Component
function HRUserTrainingDashboard() {
  // State Declarations
  const [trainings, setTrainings] = useState([]); // Store HR training data
  ...
  useEffect(() => {
    ...
    const fetchTrainings = async () => {
      // Fetch all HR trainings from Firestore
      const trainingsSnapshot = await getDocs(collection(db, "HRTrainings"));
  ...
  // Handle Submit Function to Add New HR Training
  const handleSubmit = async () => {
    // Validate the form before submission
    if (!validateForm()) return;

    try {
      // Add new HR training to Firestore
      const docRef = await addDoc(collection(db, "HRTrainings"), {
        trainingTitle,
        trainingDescription,
        schedule,
        createdBy,
      ...
      toast.success("HR Training added successfully!");
      setShowModal(false); // Close modal after successful addition
    } catch (error) {
      // Provide error feedback
      toast.error("Error adding HR training: " + error.message);
    }
  };
...
export default HRUserTrainingDashboard;
