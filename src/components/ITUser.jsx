...
const handleQuizStartClick = async () => {
  if (selectedCourse) {
    try {
      // Fetch quiz data from the enrollment (assuming it's an async function)
      await fetchQuizDataFromEnrollment(selectedCourse.id);
      
      // Close the My Courses modal
      ...
      console.error('Error fetching quiz data:', error);
      // Handle any error gracefully, maybe show an error message or modal
    }
  }
};
  ...
  <button
    onClick={() => {
      if (quizResult) {
        ...
        fetchQuizDataFromEnrollment(selectedCourse.id);
        ...
        handleQuizStartClick (); // Optional: If you have another function for general navigation
      }
    }}
  >
    Next
