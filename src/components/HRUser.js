...
function HRUserDashboard() {
  ...
  const fetchEnrolledCourses = async (userId) => {
    const enrollmentSnapshot = await getDocs(query(collection(db, "HREnrollments"), where("userId", "==", userId)));
  ...
  const fetchCompletedCourses = async (userId) => {
    const resultsSnapshot = await getDocs(query(collection(db, "HRQuizResults"), where("userId", "==", userId), where("passed", "==", true)));
  ...
  const confirmEnroll = async () => {
    if (enrollingCourse) {
      try {
        await setDoc(doc(db, "HREnrollments", `${userId}_${enrollingCourse.id}`), {
          userId,
  ...
  const handleSubmitQuiz = async () => {
    ...
    if (percentage >= 80) {
      ...
      try {
        await setDoc(doc(db, "HRQuizResults", `${userId}_${selectedCourse.id}`), resultData);
